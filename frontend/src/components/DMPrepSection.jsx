...
function DMPrepSection({ campaignId, npcs = [], locations = [], runs = [] }) {
  ...
  const fetchPrepData = async () => {
    try {
      const res = await fetch(`${API_BASE}/campaigns/${campaignId}/dm-prep`)
  ...
  const handleAddNote = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/campaigns/${campaignId}/dm-prep/note`, {
        method: 'POST',
  ...
  const handleUpdateNote = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/campaigns/${campaignId}/dm-prep/note/${editingNote.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          content: noteContent.trim(),
  ...
  const handleDeleteNote = async (noteId) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/campaigns/${campaignId}/dm-prep/note/${noteId}`, {
        method: 'DELETE'
      })
  ...
  const handleDeletePin = async (pinId) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/campaigns/${campaignId}/dm-prep/pin/${pinId}`, {
        method: 'DELETE'
      })
  ...
  const handlePinInsight = async (content, category = 'general', relatedTo = null) => {
    try {
      const res = await fetch(`${API_BASE}/campaigns/${campaignId}/dm-prep/pin`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          content,
